...
// 상품 입력화면 insert
public class PtableInsert {
	private JTextField t1;
	private JTextField t2;
	private JTextField t3;

	public PtableInsert(int x, int y, String m_id) throws Exception {
		...
		// 텍스트 필드에 입력한 값을 DTO에서 받아 insert문 실행하는  ActionListener
		JButton btnNewButton = new JButton("정보추가");
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					ptableDAO.insert(ptableDTO);
					f.dispose();
					// 상품 추가 후 상품 리스트(PtableList 클래스) 화면으로 이동
		...
		// 상품리스트(PtableList 클래스) 화면으로 이동하는 ActionListener
