...
public class TtableManagement {
	private JTextField tname;
		
	public TtableManagement(int x, int y, String m_id, String t_id) throws Exception{
...
//////////------------- 값 가져오기  ------------- //////////	
		...
		ttid.setText(ttableDTO.getT_id());
		trid.setText(ttableDTO.getR_id());
		tname.setText(ttableDTO.getR_name());
		tstime.setText(ttableDTO.getStime());
		tftime.setText(ttableDTO.getFtime());
		tbtime.setText(ttableDTO.getBtime());
		
		
//////////------------- 수정하기 버튼  ------------- //////////	
		
		
		JButton insertbtn = new JButton("수정하기");
		insertbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					
				ttableDTO.setStime(tstime.getText());
				ttableDTO.setFtime(tftime.getText());
				ttableDTO.setBtime(tbtime.getText());
			
				try {
					ttableDAO.update(ttableDTO);
					JOptionPane.showMessageDialog(null, "수정되었습니다.");
					TtableInfo name = new TtableInfo(f.getLocation().x, f.getLocation().y, m_id, ttableDTO.getR_name());
					f.dispose();
		...
		button.setBounds(819, 373, 97, 36);
		panel_3.add(button);
